...
MoveTool::MoveTool(Game* _game) : Tool(_game) {
	drag = NULL;
}
...
void MoveTool::OnPress(Entity* _entity, sf::Vector2f _pos, sf::Mouse::Button _button)
{
...
void MoveTool::OnRelease(Entity* _entity, sf::Vector2f _pos, sf::Mouse::Button _button)
{
	if (_button == sf::Mouse::Button::Left) {
		// Stop dragging it.
		drag = NULL;
...
void MoveTool::Update(float _deltatime) {
	if (drag != NULL) {
